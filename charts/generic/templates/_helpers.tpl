...
machine:
  ...
  {{- include "talm.guard.time_servers" . }}
...
cluster:
  ...
  controllerManager:
    extraArgs:
      {{- include "talm.render.component_args" (dict "args" . "field" "extraControllerManagerArgs") }}
  {{- end }}
  {{- with .Values.extraSchedulerArgs }}
  scheduler:
    extraArgs:
      {{- include "talm.render.component_args" (dict "args" . "field" "extraSchedulerArgs") }}
  {{- end }}
  {{- /* apiServer is emitted unconditionally, even with both children
         unset: it is the container an operator adds certSANs or
         extraArgs to, and TestContract_Cluster_GenericApiServerBlock
         ExistsButEmpty pins that. A bare `apiServer:` is a null value
         Talos accepts. */}}
  apiServer:
    {{- with .Values.extraApiServerArgs }}
    extraArgs:
      {{- include "talm.render.component_args" (dict "args" . "field" "extraApiServerArgs") }}
    {{- end }}
    {{- with .Values.certSANs }}
    certSANs:
    {{- toYaml . | nindent 4 }}
...
{{- include "talm.validate_floatingIP" . }}
  network:
    ...
    {{- $seenVipIPs := list }}
    {{- if $fipIsSet }}
    {{- $seenVipIPs = append $seenVipIPs (ipCanonical $fipStr) }}
    {{- end }}
    {{- range .Values.vips }}
    ...
    {{- end }}
    {{- if has (ipCanonical (.ip | toString)) $seenVipIPs }}
    ...
    {{- $seenVipIPs = append $seenVipIPs (ipCanonical (.ip | toString)) }}
    - interface: {{ .link }}
      vip:
        ip: {{ .ip }}
    {{- end }}
