...
// Contract: a component flag takes a single scalar value. The nil check
// alone let a nested map through, and toString put the Go rendering of
// the value (map[a:b]) onto the component command line.
func TestContract_Cluster_ExtraArgsNonScalar_Fails_Cozystack(t *testing.T) {
	fields := []string{"extraApiServerArgs", "extraControllerManagerArgs", "extraSchedulerArgs"}
		...
		for shape, value := range values {
			t.Run(field+"/"+shape, func(t *testing.T) {
				err := renderCozystackExpectError(t, simpleNicLookup(), map[string]any{
					field: map[string]any{"some-flag": value},
				})
				if err == nil {
					t.Fatalf("expected a fail-fast for a %s value under %s", shape, field)
				}
...
// Contract: the generic preset shares the emission, so it shares the guard.
func TestContract_Cluster_ExtraArgsNonScalar_Fails_Generic(t *testing.T) {
	err := renderGenericExpectError(t, simpleNicLookup(), map[string]any{
		"extraApiServerArgs": map[string]any{"some-flag": map[string]any{"nested": "value"}},
	})
	if err == nil {
		t.Fatal("expected a fail-fast for a non-scalar extraApiServerArgs value")
	}
...
// Contract: machine.time.servers is a list of NTP hosts. A mapping there
// renders as a mapping and the node rejects it.
func TestContract_Cluster_TimeServersMapping_Fails(t *testing.T) {
	for _, render := range []func(*testing.T, func(string, string, string) (map[string]any, error), map[string]any, ...string) error{
		renderCozystackExpectError,
		renderGenericExpectError,
	} {
		err := render(t, simpleNicLookup(), map[string]any{
			"timeServers": map[string]any{"primary": "time.cloudflare.com"},
		})
		if err == nil {
			t.Fatal("expected a fail-fast for a timeServers mapping")
		}

		if !strings.Contains(err.Error(), "timeServers") {
