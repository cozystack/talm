...
// Contract: registryTLS is keyed by the endpoint host, and an endpoint on
// an IPv6 literal carries the bracketed host:port form Talos itself
// documents. `[` opens a YAML flow sequence, so the key has to be emitted
// quoted on both schemas — pinned by decoding the render, not by matching
// a substring.
func TestContract_Machine_RegistryTLS_IPv6EndpointHost_Multidoc_Cozystack(t *testing.T) {
	out := renderCozystackWith(t, simpleNicLookup(), map[string]any{
		"advertisedSubnets": []any{testAdvertisedSubnet},
		"registryTLS": map[string]any{
			"[2001:db8::1]:5000": map[string]any{"insecureSkipVerify": true},
		},
	})
	for _, doc := range strings.Split(out, "\n---\n") {
		var v any
...
func TestContract_Machine_RegistryTLS_IPv6EndpointHost_Legacy_Cozystack(t *testing.T) {
	out := renderLegacyChart(t, cozystackChartPath, "cozystack/templates/controlplane.yaml", simpleNicLookup(), map[string]any{
		"advertisedSubnets": []any{testAdvertisedSubnet},
		"registryTLS": map[string]any{
			"[2001:db8::1]:5000": map[string]any{"insecureSkipVerify": true},
	...
	var v any
	if err := yaml.Unmarshal([]byte(out), &v); err != nil {
		t.Fatalf("legacy render with an IPv6 endpoint host must be valid YAML, got %v", err)
	}
}

// Contract: the ordinary host:port form stays bare — toYaml quotes only
// where YAML requires it, so the readable output operators are used to
// does not regress into everything-quoted.
func TestContract_Machine_RegistryTLS_PlainHostStaysBare_Multidoc_Cozystack(t *testing.T) {
	out := renderCozystackWith(t, simpleNicLookup(), map[string]any{
		"advertisedSubnets": []any{testAdvertisedSubnet},
		"registryTLS": map[string]any{
			"mirror.example.invalid:5000": map[string]any{"insecureSkipVerify": true},
		},
	})
