...
func checkSupersededFields(rendered []byte, talosVersion string) error {
	...
	conflict := errors.Wrap(
		errors.Join(conflicts...),
		"rendered config mixes v1alpha1 fields with the documents that superseded them",
	)

	// Above the boundary the contract is what moved the settings, so lowering it
	// resolves the conflict. At or below it the contract is innocent: something
	// in the charts or the patches wrote both shapes, and telling the operator
	// to pin lower sends them somewhere they already are.
	versionContract, err := renderContract(talosVersion)
	if err != nil || !versionContract.MultidocKubernetesConfigSupported() {
		//nolint:wrapcheck // cockroachdb/errors.WithHint at boundary.
		return errors.WithHintf(
			conflict,
			"templateOptions.talosVersion is %s, which still keeps these settings in v1alpha1. "+
				"Something in the charts or the patches writes both shapes: drop the v1alpha1 field "+
				"the reported document supersedes.",
			pinned,
		)
	}
