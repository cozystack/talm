...
// Contract: the legacy duplicate-ip check compares addresses, not
// strings, so one IPv6 VIP spelled two ways is still caught. Legacy
// pins at most one inline vip per interface, so a duplicate that slips
// through ships two interfaces claiming the same address.
func TestContract_NetworkLegacy_MultiVIP_DuplicateIPAcrossSpellings_Fails_Cozystack(t *testing.T) {
	err := renderCozystackExpectError(t, simpleNicLookup(), map[string]any{
		"advertisedSubnets": []any{testAdvertisedSubnet},
		"floatingIP":        "2001:0DB8::99",
		"vipLink":           "eth1",
		"vips": []any{
			map[string]any{"link": "eth2", "ip": "2001:db8::99"},
		},
	}, "v1.11")
	if err == nil {
		t.Fatal("expected a fail-fast for one VIP declared under two spellings on legacy")
	}
